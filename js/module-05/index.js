...
const getUsersByFriend = (arr, name) => {
  return arr.filter(user => user.friends.find(friend => friend === name)).map(user => user.name);
};
...
const getAllUniqueSkills = arr => {
  return arr.reduce((acc, user) => acc.concat(user.skills), []);
};

const getAllSkills = arr => {
  return getAllUniqueSkills(arr).filter((skill, i, set) => set.indexOf(skill) === i).sort();
}
